...
namespace Platformer
{
    public static class ParticleManager
    {
        ...
        public static void CreateParticleExplosionFromEntityTexture(Entity E, Rectangle TexelField, float GravForce, float DriftSpeed, bool FlipHorz, bool Collision, bool Anim)
        {
                ...
                for (int iy = TexelField.Y; iy < TexelField.Height + TexelField.Y; iy++)
                {
                    ...
                    if (Col2D[ix, iy].A != 0 || Col2D[ix, iy].ToVector3() != Vector3.Zero)
                    {
                        ...
                        PArray[(int)Point.X + (int)Point.Y * Col2D.GetLength(1) - 1] = new Particle(Point * PixelSize + E.GetPosVector2(),
                            ...
                            // Friction                Size
                            new Vector2(1.02f, 1.003f), new Vector2(PixelSize), Collision);
                    }
                }
            }
        ...
        public static void CreateParticleExplosionFromEntityTexture(Entity E, Rectangle TexelField, float GravForce, float DriftSpeed, bool FlipHorz, bool Collision, bool Anim, Vector2 StartSpeed)
        {
        ...
        public static void CreateExplosion(Vector2 Pos, Vector2 Vel, Color Col)
        {
            ...
            for (int ip = 0; ip < 20; ip++)
            {
                ...
                PArray[ip] = new Particle(Pos, new Vector2((float)Math.Sin(Angle) * Strength, 
                    (float)Math.Cos(Angle) * Strength) + Vel + new Vector2(0, -2), Col, 0.02f, 50 + Values.RDM.Next(30));
            }
            AddParticleBatch(new ParticleBatch(PArray, Pos, 0, 0, 0, 0));
        }
        public static void CreateDescendingParticleOfEntity(Entity E, Rectangle TexelField, float GravForce, bool FlipHorz)
        {
            Particle[] PArray = new Particle[1];
            PArray[0] = new Particle(E.GetPosVector2() + E.GetSizeVector2() / 2, 0, E.Texture, Color.White, E.Vel + new Vector2(0, -3), 0.3f, new Vector2(1.02f, 1.003f), E.GetSizeVector2() / 15, false);
